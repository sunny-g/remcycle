...
import { of } from 'most';
import withActions from '../../src/hoc/withActions';
...
describe('withActions HOC', () => {

  test('should emit actions transformed by specific mapper', done => {
    const Main = withActions({
      [TYPE1]: {
        [TYPE2]: action => type2(!action.payload)
      },
    })(main);
  ...
  test('should emit all actions defined in mappers', done => {
    ...
    const Main = withActions({
      [TYPE1]: {
        [TYPE2]: action => type2(action.payload + 10),
        [TYPE3]: action => type2(action.payload + 100)
      },
    })(main);
  ...
  test('should provide props to mapper', done => {
    const Main = withActions({
      [TYPE1]: {
        [TYPE2]: (action, { type1 }) => ({ ...action, payload: action.payload && type1 })
      },
    })(main);
  ...
  test('should provide latest props to mapper', done => {
    ...
    const Main = withActions({
      [TYPE1]: {
        [TYPE2]: (action, { type1 }) => type2(action.payload + type1),
      },
    })(main);
